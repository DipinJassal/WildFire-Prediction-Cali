...
counties = (
    cousub
    .dissolve(by="COUNTYFP", as_index=False)
...
joined = gpd.sjoin(gdf_fires, counties[["county", "geometry"]], how="left", predicate="within")
...
print(f"Unmatched after 'within' join: {unmatched_count} ({unmatched_count/len(joined)*100:.2f}%)")
...
keep_cols = [
    ...
    "satellite", "confidence", "bright_t31", "frp", "daynight", "type", "county",
]
...
base[["max_frp", "max_brightness", "fire_count"]] = (
    base[["max_frp", "max_brightness", "fire_count"]].fillna(0)
)
...
total = len(base)
log(f"\n2. Fire label distribution:")
log(f"   fire_label=1 (fire day):    {fire_counts.get(1, 0):>7,}  ({fire_counts.get(1, 0)/total*100:.2f}%)")
log(f"   fire_label=0 (no fire day): {fire_counts.get(0, 0):>7,}  ({fire_counts.get(0, 0)/total*100:.2f}%)")
...
log(f"\n4. Top 10 counties by total fire days:")
county_fire = base[base["fire_label"] == 1].groupby("county").size().sort_values(ascending=False)
